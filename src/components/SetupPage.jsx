...
import { needsPasswordSetup, setupOwner } from '../lib/auth'
...
export default function SetupPage() {
  ...
  const onSubmit = async (e) => {
    ...
    setBusy(true)
    try {
      const result = await setupOwner(username, password, confirm)
      if (!result.ok) {
        setError(result.error || t('auth.setupError'))
        return
      }
      showSaveStatus('Owner account ready.', 'success')
      ...
      <div className="section-inner">
        <ScrollReveal>
          <div className="mx-auto w-full max-w-lg card-surface p-6 sm:p-8">
            ...
            <h1 className="heading-display mt-2 text-3xl text-gradient-brand">{t('auth.setupTitle')}</h1>
            <p className="mt-3 text-sm text-ink-muted">{t('auth.setupIntro')}</p>
